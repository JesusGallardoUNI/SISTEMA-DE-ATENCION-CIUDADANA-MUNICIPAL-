<?php
    ...
    if($_SERVER["REQUEST_METHOD"] === "POST"){
        ...
        $Query = "SELECT r.codigo_postal, r.nombre_colonia, r.tipo_reporte, r.especificacion, r.fecha, rr.fecha_resuelto FROM reportes_colonias r LEFT JOIN reportes_resueltos rr ON r.clave = rr.clave WHERE r.resuelto = '$Resuelto' AND r.fecha BETWEEN '$Inicio' AND '$Fin';";    
                ...
                <?php if (isset($Ejecuta) && $Ejecuta && mysqli_num_rows($Ejecuta) > 0): ?>
                    <?php while ($Registro = mysqli_fetch_assoc($Ejecuta)): ?>
                        <li>
                            ...
                            <td>Colonia: <?php echo $Registro['nombre_colonia'] ?? ""; ?>.</td>
                            <td>Grupo: <?php echo Traductor($Registro['tipo_reporte']) ?? ""; ?>.</td>
                            <td>Reporte o servicio: <?php echo $Registro['especificacion'];?>.</td>
